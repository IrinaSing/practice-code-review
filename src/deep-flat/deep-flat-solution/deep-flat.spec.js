...
describe('flattens arrays with different levels of depths', () => {
  ...
  describe('flattens array with 3 > levels of depth', () => {
    ...
    it('array with strings', () => {
      const expected = [
        ...
        'typo',
        'j',
        'aaa', // eslint-disable-line
      ];
      const received = deepFlat([
        'cat',
        'robot',
        ['a', ['g'], [['!?', ['%winter'], ['u', ['typo']]], 'j'], 'aaa'], // eslint-disable-line
      ]);
      expect(received).toEqual(expected);
    });
  });
  describe('removes empty slot', () => {
    it('array with numbers', () => {
      ...
      const received = deepFlat([45, 253, [5, , 9]]); // eslint-disable-line
      expect(received).toEqual(expected);
    });
  });
  describe('There should be no side-effects', () => {
    it('no side-effects', () => {
      const anArray = [0, [1, -1, 2], -2];
      const received = deepFlat(anArray) !== anArray; // eslint-disable-line
